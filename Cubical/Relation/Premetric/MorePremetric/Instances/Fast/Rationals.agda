module Cubical.Relation.Premetric.MorePremetric.Instances.Fast.Rationals where
...
isPremetric (snd ℚPremetricSpace) = isPMℚ
  where
    ...
    isPMℚ .isRefl≈ x ε   = subst ((_< ⟨ ε ⟩₊) ∘ abs) (sym (+InvR x)) (uncurry ℚ₊.0<ᵗ→< ε)
    ...
    isPMℚ .isRounded≈ x y ε <ε = ∣_∣₁ $
      let
        δ : ℚ₊
        δ = mean (abs(x - y)) ⟨ ε ⟩₊ , (ℚ₊.<→0<ᵗ (mean (abs(x - y)) ⟨ ε ⟩₊) $ begin<
          0                        ≤⟨ 0≤abs (x - y) ⟩
