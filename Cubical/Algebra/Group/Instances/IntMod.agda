...
1-ₘsuc n y =
   ...
   ∙ cong -ₘ_
      (GroupTheory.invDistr (ℤGroup/ (suc n))
        (1 mod suc n , mod< n 1) (-ₘ (suc y mod suc n , mod< n (suc y)))
      ∙ cong (_-ₘ (1 mod suc n , mod< n 1))
       (GroupTheory.invInv (ℤGroup/ (suc n)) (suc y mod suc n , mod< n (suc y)))
       ∙ suc-ₘ1 n y)
...
isHomℤ→Fin n =
  makeIsGroupHom
    λ { (pos x) y → pos+case x y
      ...
      ; (negsuc x) (negsuc y) →
        ...
        ∙∙ GroupTheory.invDistr (ℤGroup/ (suc n))
             (suc x mod suc n
            , mod< n (suc x)) (suc y mod suc n , mod< n (suc y))
  ...
  +1case (negsuc zero) =
      Σ≡Prop (λ _ → isProp≤) refl
    ∙ sym (GroupStr.·InvR (snd (ℤGroup/ (suc n))) (1 mod suc n , mod< n 1))
  +1case (negsuc (suc y)) =
    Σ≡Prop (λ _ → isProp≤)
  ...
  pos+case (suc (suc x)) y =
    ...
    ∙∙ +1case (pos (suc x) +ℤ y)
    ∙∙ (cong ((1 mod suc n , mod< n 1) +ₘ_) (pos+case (suc x) y)
     ∙∙ sym (+ₘ-assoc (1 mod suc n , mod< n 1)
              (suc x mod suc n , mod< n (suc x)) (ℤ→Fin n y))
     ∙∙ cong (_+ₘ ℤ→Fin n y) (lem x))
    where
    lem : (x : ℕ)
      → (1 mod suc n , mod< n 1) +ₘ (suc x mod suc n , mod< n (suc x))
