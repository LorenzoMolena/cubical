...
-- Trichotomy of <ᵗ can be alternatively proven by induction:
-- NOTE: Don't remove it, this alternative implementation is needed in few modules

_≟ᵗ'_ : ∀ m n → Trichotomyᵗ m n
zero ≟ᵗ' zero = eq refl
zero ≟ᵗ' suc n = lt tt
suc m ≟ᵗ' zero = gt tt
...
module Minimal where
  ...
  ... | inl (m , P1+m , ¬P<1+n) = inl $ suc m , P1+m , λ
    { zero    → λ _ → ¬P0
    ; (suc k) → ¬P<1+n k }
  ... | inr ¬P<1+n = inr λ
    { zero    → λ _ → ¬P0
  ...
  Least-unique m n (Pm , ¬P<m) (Pn , ¬P<n) with m ≟ᵗ n
