...
min-fComm : ∀ n m → min n m ≡ min m n
min-fComm (pos m)    (pos n)    = cong pos (ℕ.minComm m n)
min-fComm (pos m)    (negsuc n) = refl
min-fComm (negsuc m) (pos n)    = refl
min-fComm (negsuc m) (negsuc n) = cong negsuc (ℕ.maxComm m n)
...
min-fIdem : ∀ n → min n n ≡ n
min-fIdem (pos n)    = cong pos (ℕminIdem n)
min-fIdem (negsuc n) = cong negsuc (ℕmaxIdem n)
...
max-fComm : ∀ m n → max m n ≡ max n m 
max-fComm (pos m)    (pos n)    = cong pos (ℕ.maxComm m n)
max-fComm (pos m)    (negsuc n) = refl
max-fComm (negsuc m) (pos n)    = refl
max-fComm (negsuc m) (negsuc n) = cong negsuc (ℕ.minComm m n)

max-fIdem : ∀ n → max n n ≡ n 
max-fIdem (pos n)    = cong pos (ℕmaxIdem n)
max-fIdem (negsuc n) = cong negsuc (ℕminIdem n)
...
minSucR m = min-fComm m (sucℤ m) ∙ minSucL m
...
minPredR m = min-fComm m (predℤ m) ∙ minPredL m
...
maxSucR m = max-fComm m (sucℤ m) ∙ maxSucL m
...
maxPredR m = max-fComm m (predℤ m) ∙ maxPredL m

-- tests with transport / hcomp

-- +Comm : ∀ m n → m + n ≡ n + m
-- +Comm m n = subst-f
--   (λ _+_ _·_ → m + n ≡ n + m) (P.+Comm m n)

-- +Comm : ∀ m n → m + n ≡ n + m
-- +Comm m n i = hcomp (λ j → λ { (i = i0) → +≡+f m n j ;
--                                (i = i1) → +≡+f n m j })
--                     (P.+Comm m n i)
...
-- +Assoc m n o i = hcomp (λ j → λ { (i = i0) → +≡+f m (+≡+f n o j) j ;
--                                   (i = i1) → +≡+f (+≡+f m n j) o j })
--                        (P.+Assoc m n o i)

-- test, with only two cases not transported
+Assoc : ∀ m n o → m + (n + o) ≡ (m + n) + o
+Assoc (pos m)    (pos n)    (pos o)    = cong pos (ℕ.+-assoc m n o)
+Assoc (pos m)    (pos n)    (negsuc o) = let
  x = pos m
  y = pos n
  z = negsuc o
  in
  subst-f (λ _+_ _·_ → x + (y + z) ≡ (x + y) + z) (P.+Assoc x y z)
+Assoc (pos m)    (negsuc n) (pos o)    = let
  x = pos m
  y = negsuc n
  z = pos  o
  in
  subst-f (λ _+_ _·_ → x + (y + z) ≡ (x + y) + z) (P.+Assoc x y z)
+Assoc (pos m) (negsuc n) (negsuc o)    = let
  x = pos m
  y = negsuc n
  z = negsuc o
  in
  subst-f (λ _+_ _·_ → x + (y + z) ≡ (x + y) + z) (P.+Assoc x y z)
+Assoc (negsuc m) (pos n)    (pos o)    = let
  x = negsuc m
  y = pos n
  z = pos o
  in
  subst-f (λ _+_ _·_ → x + (y + z) ≡ (x + y) + z) (P.+Assoc x y z)
+Assoc (negsuc m) (pos n)    (negsuc o) = let
  x = negsuc m
  y = pos n
  z = negsuc o
  in
  subst-f (λ _+_ _·_ → x + (y + z) ≡ (x + y) + z) (P.+Assoc x y z)
+Assoc (negsuc m) (negsuc n) (pos o)    = let
  x = negsuc m
  y = negsuc n
  z = pos o
  in
  subst-f (λ _+_ _·_ → x + (y + z) ≡ (x + y) + z) (P.+Assoc x y z)
+Assoc (negsuc m) (negsuc n) (negsuc o) = cong (negsuc ∘ suc) (
  m +ℕ (1 +ℕ (n +ℕ o)) ≡⟨ ℕ.+-assoc m 1 _ ⟩
  m +ℕ 1 +ℕ (n +ℕ o)   ≡⟨ cong (_+ℕ (n +ℕ o)) (ℕ.+-comm m 1) ⟩
  1 +ℕ m +ℕ (n +ℕ o)   ≡⟨ sym (ℕ.+-assoc 1 m (n +ℕ o)) ⟩
  1 +ℕ (m +ℕ (n +ℕ o)) ≡⟨ cong (1 +ℕ_) (ℕ.+-assoc m n o) ⟩
  suc (m +ℕ n +ℕ o)    ∎)

  -- subst-f
  -- (λ _+_ _·_ → m + (n + o) ≡ (m + n) + o) (P.+Assoc m n o)
